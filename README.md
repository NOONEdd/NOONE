...
1. **Workers & Pages → KV → Create a namespace** — name it anything, e.g. `academy-coach-data`.
...
1. **Nyx NOONEdd Academy effective data** — champions/items/runes/builds/matchups/decision trees, resolved from `src/data/*.js` (static baseline) merged with live Coach Mode KV overrides. This is the primary source for essentially everything the AI says about the game.
